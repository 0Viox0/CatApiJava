...
@Component
public class RemoveCatFromUserHandler extends HandlerBase {

    private final UserService userService;
    private final MessagingMapper messagingMapper;
    private final ObjectMapper objectMapper;

    public RemoveCatFromUserHandler(
    ...
    public MessageModel handle(MessageModel message) {
        if (message.getOperation().equalsIgnoreCase("deleteCat")) {
            try {
                UserResponseDto userResponseDto = userService.deleteCat(
